...
class SimpleImage
{
    ...
    public const ERR_INVALID_POSITION = 13;
    ...
    /*
    width = thumbnail width without scaled
    height = thumbnail height without scaled
    positionX = x position of the thumbnail from center (scaled)
    positionY = y position of the thumbnail from center (scaled)
    scale = scale of the thumbnail
    */
    public function specialThumbnail(int $width, int $height, float $positionX, float $positionY, float $scale)
    {
        // Determine aspect ratios
        $currentRatio = $this->getHeight() / $this->getWidth();
        $targetRatio = $height / $width;
        ...
        // Fit to height/width
        if ($targetRatio > $currentRatio) {
            $this->resize(null, $height * $scale);
        } else {
            $this->resize($width * $scale);
        }

        $x1 = $this->getWidth() / 2 - $width / 2 - $positionX;
        if (floor($x1) == -1) {
            $x1++;
        } else if (ceil($x1) == $this->getWidth() + 1) {
            $x1--;
        }
        $x2 = $x1 + $width;
        $y1 = $this->getHeight() / 2 - $height / 2 - $positionY;
        if (floor($y1) == -1) {
            $y1++;
        } else if (ceil($y1) == $this->getHeight() + 1) {
            $y1--;
        }
        $y2 = $y1 + $height;

        if ($x1 < 0 || $x2 > $this->getWidth() || $y1 < 0 || $y2 > $this->getHeight()) {
            throw new \Exception("The position is out of the image.", self::ERR_INVALID_POSITION);
        }
